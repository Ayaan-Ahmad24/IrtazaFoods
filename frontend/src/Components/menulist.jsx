...
const MenuPage = () => {
    ...
    const getQuantityText = (quantity, name) => {
        ...
        if (quantity === 1 && name.toLowerCase().includes("sharbat")) return "1 Bottle";
    ...
    const handlePurchaseClick = (itemName, itemId) => {
        const productLink = `http://localhost:5000/product/${itemId}`; // Adjust based on your product page URL
    ...
    return (
        <div className="bg-white text-black font-sans pt-[calc(var(--header-height, 0px) + 1rem)] pb-10 mt-32">
            ...
            <section className="container mx-auto py-10 px-4">
                <h2 className="text-3xl font-bold mb-6 text-yellow-600 text-center">Menu Items</h2>
                        ...
                        menuItems.map(item => (
                            ...
                            >
                                <div className="overflow-hidden rounded-lg h-2/3">
                                    <img src={item.image} alt={item.name} className="w-full h-[250px] object-cover mb-4 transition-transform transform hover:scale-110" />
                                </div>
                                <h3 className="text-xl font-bold text-yellow-600 transition-transform transform hover:scale-105">{item.name}</h3>
                                <p className="text-lg text-yellow-500 font-semibold">{item.price} /-</p>
                                <p className="text-sm text-gray-700">{getQuantityText(item.quantity, item.name)}</p>
                                <button
