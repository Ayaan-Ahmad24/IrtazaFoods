...
const BlogListPage = () => {
    ...
    useEffect(() => {
        ...
        const fetchBlogs = async () => {
            try {
                const response = await axios.get('http://localhost:5000/api/blog/get-blog');
    ...
    return (
        <div className="bg-white text-black font-sans pt-[calc(var(--header-height, 0px) + 1rem)] pb-10 mt-32">
            <section className="container mx-auto py-10 px-4" data-aos="fade-up">
                <h2 className="text-3xl font-bold mb-6 text-yellow-600 text-center">Blog List</h2>
                {isLoading ? (
                        ...
                        {blogs.length ? (
                            blogs.map(blog => (
                                <div
                                    ...
                                    data-aos="fade-up"
                                >
                                    <h3 className="text-xl font-bold text-yellow-600">{blog.title}</h3>
